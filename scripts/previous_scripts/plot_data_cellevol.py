...
def blob_plot(ax,data,list_pos, colour,datatype,extra=False):
    ...
    for d,p in zip(data,list_pos):
      ...
      if datatype=='int':
        his, bins = np.histogram(d, bins=[ 5*x for x in range(2+ M/5 ) ])
        #if len(d)>4:
          #print bins
          #print his
...
ltime=[]
l_avrgdata_time=[]
lcounter=np.zeros((3,3))
lJmatrix=np.zeros((3,3))

l_avrgdata_time2=[]
lcounter2=np.zeros((3,3)) #version 2 keeps count of each single value,so that proportions can be made for blob or scatter plot 
                                    # *** actually this might be useless, as blobplot funtion makes internal histogram

lJmatrix2 = [[[] for j in range(3)] for i in range(3)]
...
with open(filename,"r") as fin:
      for line in fin:
        ...
        lpop[tau]+=1 
        for ctau, cJ in [ contacts[pos:pos + 2] for pos in range(0, len(contacts), 2) ]:
          ctau=int(ctau)
          lcounter[tau,ctau]+=1
          print("cJ is {}".format(cJ))
